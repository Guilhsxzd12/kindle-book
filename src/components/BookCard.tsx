...
export function BookCard({book,isAdmin=false}:{book:Book;isAdmin?:boolean}){
  ...
  return <div className={"book-card-admin-wrap"+(isAdmin?" is-admin":"")}>
    <Link className="book-card catalog-book-card" href={bookPath(book.slug)}>
      <div className="book-cover-wrap">{coverUrl?<img className="cover" src={coverUrl} alt={`Capa de ${book.title}`}/>:<div className="cover-fallback">{book.title}</div>}{book.categories?.name&&<span className="floating-category">{book.categories.name}</span>}</div>
