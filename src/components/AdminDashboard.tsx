...
export function AdminDashboard({initialBooks,initialCoverlessBooks,initialCategories,initialProfiles,initialRequests}:Props){
  ...
  const normalizedBookSearch=normText(bookSearch);
  const shownBooks=normalizedBookSearch?books.filter(book=>normText(`${book.title} ${book.author} ${book.categories?.name||""}`).includes(normalizedBookSearch)):books.slice(0,8);
  const normalizedCoverSearch=normText(coverSearch);
  const shownCoverlessBooks=normalizedCoverSearch?coverlessBooks.filter(book=>normText(`${book.title} ${book.author} ${book.categories?.name||""}`).includes(normalizedCoverSearch)):coverlessBooks.slice(0,12);
  ...
  async function removeBook(id:string){if(!confirm("Excluir este livro do catálogo?"))return;const response=await fetch(`/api/admin/books?id=${encodeURIComponent(id)}`,{method:"DELETE"});const data=await response.json();if(response.ok){setBooks(current=>current.filter(book=>book.id!==id));setCoverlessBooks(current=>current.filter(book=>book.id!==id));setMessage("Livro excluído.");}else setMessage(data.error||"Não foi possível excluir.");}
  ...
  async function saveCover(book:Book){
    ...
    try{
      ...
      setCoverlessBooks(current=>current.filter(item=>item.id!==book.id));setBooks(current=>current.map(item=>item.id===book.id?data.book:item));cancelCoverEdit();setMessage(`Capa de ${data.book.title} adicionada com sucesso.`);
  ...
  return <>
    ...
    {tab==="editar"&&<div className="admin-grid">
      ...
      <section className="card panel admin-library-panel"><div className="panel-title"><div><span className="eyebrow">CORREÇÕES MANUAIS</span><h2>Livros com dados faltantes</h2></div><span className="count-badge">{books.length}</span></div>
        ...
        <p className="admin-list-hint">{normalizedBookSearch?`${shownBooks.length} ${shownBooks.length===1?"resultado encontrado":"resultados encontrados"}`:`Mostrando ${Math.min(8,books.length)} livros com capa, sinopse, autor ou título faltando`}</p>
