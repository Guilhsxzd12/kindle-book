...
export function AdminDashboard({initialBooks,initialCoverlessBooks,initialCategories,initialProfiles,initialRequests}:Props){
  ...
  return <>
    ...
    {tab==="editar"&&<div className="admin-grid">
      ...
      <section className="card panel admin-library-panel"><div className="panel-title"><div><span className="eyebrow">CORREÇÕES MANUAIS</span><h2>Livros com dados faltantes</h2></div><span className="count-badge">{books.length}</span></div>
        <div className="admin-book-search"><input type="search" value={bookSearch} onChange={event=>setBookSearch(event.target.value)} placeholder="Pesquisar entre os livros que precisam de correção..." aria-label="Pesquisar livros cadastrados"/>{bookSearch&&<button type="button" className="btn ghost small" onClick={()=>setBookSearch("")}>Limpar</button>}</div>
        <p className="admin-list-hint">{bookSearch&& !bookSearchReady?`Digite mais ${4-normalizedBookSearch.length} ${4-normalizedBookSearch.length===1?"letra":"letras"} para pesquisar em todo o acervo.`:bookSearchBusy?"Pesquisando...":bookSearchReady?`${shownBooks.length} ${shownBooks.length===1?"resultado encontrado":"resultados encontrados"} no acervo`:`Mostrando ${Math.min(8,books.length)} livros com capa ou sinopse faltando`}</p>
