...
type Props={initialBooks:Book[];initialCoverlessBooks:Book[];initialCategories:Category[];initialProfiles:Profile[];initialRequests:BookRequest[]};
type Tab="editar"|"capas"|"pedidos"|"categorias"|"usuarios"|"drive";
...
export function AdminDashboard({initialBooks,initialCoverlessBooks,initialCategories,initialProfiles,initialRequests}:Props){
  const [tab,setTab]=useState<Tab>("editar");
  const [books,setBooks]=useState(initialBooks);const [coverlessBooks,setCoverlessBooks]=useState(initialCoverlessBooks);const [categories,setCategories]=useState(initialCategories);const [profiles,setProfiles]=useState(initialProfiles);const [requests,setRequests]=useState(initialRequests);
  ...
  useEffect(()=>{
    const query=bookSearch.trim();if(normText(query).length<4){setBookSearchResults([]);setBookSearchBusy(false);return;}
    const controller=new AbortController();const timer=setTimeout(async()=>{setBookSearchBusy(true);try{const response=await fetch(`/api/admin/book-search?q=${encodeURIComponent(query)}`,{signal:controller.signal,cache:"no-store"});const data=await response.json();if(response.ok)setBookSearchResults((data.books||[]) as Book[]);}catch{}finally{if(!controller.signal.aborted)setBookSearchBusy(false);}},220);
  ...
  function notifyAdmin(title:string,body:string){if("Notification" in window&&Notification.permission==="granted")new Notification(title,{body,icon:"/kindle-books-logo.svg"});if("vibrate" in navigator)navigator.vibrate([120,60,120]);}
  ...
  const tabs:[Tab,string][]=[["pedidos",`Pedidos (${pendingRequests.length})`],["editar","Editar livros"],["capas","Editar capas"],["categorias","Categorias"],["usuarios","Usuários"],["drive","Google Drive"]];
  return <>
    ...
    {tab==="editar"&&<div className="admin-grid">
      <section className="card panel"><div className="panel-title"><div><span className="eyebrow">{editing?"EDIÇÃO":"NOVO TÍTULO"}</span><h2>{editing?`Editar ${editing.title}`:"Adicionar livro"}</h2></div>{(editing||requestId)&&<button className="btn ghost small" onClick={resetForm}>Cancelar</button>}</div>
        ...
        <form id="book-form" className="stack" action={formData=>saveBook(formData,true)}>
          ...
          <button className="btn" disabled={busy||telegramBusyId!==null}>{busy?(progress?`Enviando livro • ${progress}%`:"Enviando livro..."):"Enviar livro"}</button>
          <small className="telegram-publish-hint">Um único envio publica o livro no site e encaminha EPUB + PDF aos canais Oficial e Reserva do Telegram.</small>
        </form>
      </section>
