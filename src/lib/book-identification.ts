...
export async function identifyBookFromUpload(fileName:string,mimeType:string,bytes:Uint8Array,hints?:{title?:string|null;author?:string|null}):Promise<IdentifiedBook>{
  ...
  const lookupAuthor=isEpub?author:((suspiciousAuthor(hints?.author)||suspiciousAuthor(author))?null:(hints?.author||author));
  const lookup=await lookupBookMetadata({title:title||guess.title,author:lookupAuthor,isbn});
