...
async function googleBooks(query:string){
  try{
    ...
    return (payload.items||[]).map((item:any):AutomaticBookMetadata=>{
      ...
      return {title:String(v.title||"").trim(),author:Array.isArray(v.authors)&&v.authors.length?v.authors.join(", "):"Autor não informado",coverUrl:(v.imageLinks?.extraLarge||v.imageLinks?.large||v.imageLinks?.medium||v.imageLinks?.thumbnail||v.imageLinks?.smallThumbnail||null)?.replace("http://","https://")||null,year:yearFrom(v.publishedDate),pages:Number.isFinite(v.pageCount)?Number(v.pageCount):null,language:normalizeLanguage(v.language),isbn:compactIsbn(isbn)||null,categories:Array.isArray(v.categories)?v.categories.slice(0,18):[],source:"google-books",score:0};
