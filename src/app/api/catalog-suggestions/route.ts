...
export async function GET(request:NextRequest){
  ...
  if(!viewer.user||!viewer.profile)return NextResponse.json({error:"Acesso negado."},{status:401});

  const q=(request.nextUrl.searchParams.get("q")||"").trim().slice(0,100);
  if(q.length<2)return NextResponse.json({items:[]});

  const {data,error}=await viewer.supabase.rpc("catalog_search_suggestions",{p_search:q,p_limit:8});
  if(error){
